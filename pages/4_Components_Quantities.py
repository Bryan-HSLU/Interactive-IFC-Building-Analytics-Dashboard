...
with tab2:
    ...
    if comp_df.empty:
        ...
        # ── Chart 5: Box Plot — Length/Area distribution per Type ────────────
        st.subheader("Distribution by Type")
        if _has_len:
            _box_col = "length_m"
            _box_label = "Length (m)"
        elif "area_m2" in comp_df.columns and pd.to_numeric(comp_df["area_m2"], errors="coerce").fillna(0).gt(0).any():
            _box_col = "area_m2"
            _box_label = "Area (m²)"
        else:
            _box_col = None
        ...
        if _box_col and "type_name" in comp_df.columns:
            st.caption(f"Box plot of {_box_label} distribution per type (top 10 by count).")
            _top10_types = comp_df["type_name"].fillna("(no type)").value_counts().head(10).index.tolist()
            _pal5 = (CATEGORICAL_COLORS * (len(_top10_types) // len(CATEGORICAL_COLORS) + 1))
            fig_c5 = go.Figure()
            for i, t in enumerate(_top10_types):
                _vals = pd.to_numeric(
                    comp_df[comp_df["type_name"].fillna("(no type)") == t][_box_col],
                    errors="coerce",
                ).dropna()
                _vals = _vals[_vals > 0]
                if not _vals.empty:
                    fig_c5.add_trace(go.Box(
                        y=_vals.tolist(),
                        name=t,
                        marker_color=_pal5[i],
                        boxpoints="outliers",
                        hovertemplate=f"<b>{t}</b><br>{_box_label}: %{{y:,.2f}}<extra></extra>",
                    ))
            if len(fig_c5.data) > 0:
                fig_c5.update_layout(
                    template="plotly_white",
                    yaxis_title=_box_label,
                    xaxis_title="Type",
                    xaxis_tickangle=-30,
                    showlegend=False,
                ...
                st.plotly_chart(fig_c5, use_container_width=True, key="ca_chart5")
            else:
                st.info(f"No {_box_label} values > 0 for the selected types.")
        else:
            st.info("No length or area data available for distribution chart.")
