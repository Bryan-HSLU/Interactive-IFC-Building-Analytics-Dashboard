...
def create_material_volume_bar(element_df: pd.DataFrame, unit: str = \"m³\") -> go.Figure:
    ...
    agg = pd.concat([andere_row, main_rows], ignore_index=True) if not andere_row.empty else main_rows
    ...
    if not main_rows.empty and total_volume > 0:
        ...
        pct = (max_row[\"quantity\"] / total_volume) * 100
        fig.add_annotation(
            x=max_row[\"quantity\"], y=max_row[\"material\"],
            text=f\"macht {pct:.1f}% des Gesamtvolumens aus\",
            showarrow=False, xanchor=\"left\", xshift=65,
    ...
    fig.update_layout(
        title=dict(text=f\"Materialmengen im Gebäude ({unit})\", font=dict(size=14, color=COLORS[\"text\"]),
...
def create_material_co2_bar(element_df: pd.DataFrame) -> go.Figure:
    ...
    if andere_co2 > 0:
        if \"Andere\" in agg_haupt[\"material\"].values:
            ...
            agg_haupt = pd.concat([pd.DataFrame([{\"material\": \"Andere\", \"co2\": andere_co2}]), agg_haupt], ignore_index=True)

    agg = agg_haupt.copy()
    is_andere = agg[\"material\"] == \"Andere\"
    agg = pd.concat([agg[is_andere], agg[~is_andere].sort_values(\"co2\", ascending=True)], ignore_index=True)
    ...
    for v in vals:
        ...
        if scaled < 0.5:
            t = scaled / 0.5
            r, g, b = int(168 + t*(245-168)), int(213 + t*(230-213)), int(181 + t*(66-181))
        else:
            t = (scaled - 0.5) / 0.5
            r, g, b = int(245 + t*(217-245)), int(230 + t*(79-230)), int(66 + t*(61-66))
    ...
    fig.add_vline(x=avg_val, line_dash=\"dash\", line_color=COLORS[\"text_light\"], line_width=1.5,
...
def create_room_co2_scatter(space_df: pd.DataFrame) -> go.Figure:
    ...
    for usage in usages:
        ...
        fig.add_trace(go.Scatter(
            x=sub[\"area_m2\"], y=sub[\"co2_load\"], mode=\"markers\", name=usage,
            marker=dict(color=color, size=14, opacity=0.95, line=dict(width=1.5, color=\"#2D2D2D\")),
            text=names, customdata=sub[\"usage\"].tolist(),
    ...
    if len(all_x) > 1:
        try:
            ...
            fig.add_trace(go.Scatter(
                x=x_range, y=m * x_range + c, mode=\"lines\", name=\"Trendlinie\",
                line=dict(color=COLORS[\"text_light\"], width=1.5, dash=\"dot\"), hoverinfo=\"skip\",
            ))
        except Exception:
            pass

    if len(all_x) > 1 and m != 0:
        candidates = [row for _, row in df.iterrows()
                      if not any(k in str(row.get(\"name\",\"\")).lower() or k in str(row.get(\"usage\",\"\")).lower()
                                 for k in [\"wc\", \"technik\"])]
        if candidates:
            ...
            annotated_ids, to_annotate = set(), []
            for idx, row in cand_df.iterrows():
                if \"veloraum\" in str(row.get(\"name\",\"\")).lower() or \"veloraum\" in str(row.get(\"usage\",\"\")).lower():
                    to_annotate.append(row); annotated_ids.add(row.name)
            for idx, row in cand_df.iterrows():
                if row.name in annotated_ids: continue
                area = float(row.get(\"area_m2\", 0))
                is_aufenthalt = any(k in str(row.get(\"name\",\"\")).lower() or k in str(row.get(\"usage\",\"\")).lower()
                                    for k in [\"saal\", \"restaurant\", \"bar\", \"empfang\", \"warteraum\", \"backstage\"])
                if is_aufenthalt and area > 100:
                    to_annotate.append(row); annotated_ids.add(row.name)
            if len(to_annotate) < 3:
                std_r = df[\"residual\"].std() if len(df) > 2 else 1.0
                if pd.isna(std_r) or std_r <= 0: std_r = 1.0
                for idx, row in cand_df[cand_df[\"residual\"] > 1.2 * std_r].sort_values(\"residual\", ascending=False).iterrows():
                    if len(to_annotate) >= 3: break
                    if row.name not in annotated_ids:
                        to_annotate.append(row); annotated_ids.add(row.name)
            ...
            for row in to_annotate:
                ...
                rn, rt = str(room_name).lower(), str(room_type).lower()
                if \"bar\" in rn or \"empfang\" in rn or \"bar\" in rt or \"empfang\" in rt: ax_offset, ay_offset = -40, 45
                elif \"saal\" in rn or \"saal\" in rt: ax_offset, ay_offset = -60, -40
                elif \"veloraum\" in rn or \"veloraum\" in rt: ax_offset, ay_offset = 60, -45
                fig.add_annotation(
                    x=row[\"area_m2\"], y=row[\"co2_load\"], text=label_text,
                    showarrow=True, arrowhead=2, arrowcolor=\"#D94F3D\", arrowsize=1.0,
                    ax=ax_offset, ay=ay_offset,
                    font=dict(size=13, color=\"#2D2D2D\", family=\"Inter, sans-serif\"),
                    bgcolor=\"rgba(253, 237, 236, 0.95)\", bordercolor=\"#FADBD8\", borderwidth=1.5, borderpad=5,
                )
...
def create_element_material_stacked_bar(element_df: pd.DataFrame) -> go.Figure:
    ...
    if element_df.empty or \"ifc_class\" not in element_df.columns or \"material\" not in element_df.columns:
        ...
        def _map_class_to_part(row):
            ...
            if cls in (\"IfcWall\", \"IfcWallStandardCase\", \"IfcCurtainWall\"): return \"Wand\"
            elif cls == \"IfcRoof\": return \"Decke\"
            elif cls == \"IfcSlab\":
                return \"Decke\" if any(k in type_name for k in [\"decke\", \"dach\", \"roof\", \"ceiling\"]) else \"Boden\"
    ...
    for grp in group_order:
        fig.add_trace(go.Bar(
            ...
            marker_color=_MATERIAL_GROUP_COLORS[grp],
            hovertemplate=f\"<b>%{{x}}</b><br>{grp}: %{{y:.1f}}%<extra></extra>\",
        ))
    fig.update_layout(barmode=\"stack\")
    apply_default_layout(fig, \"Materialanteil pro Bauteilgruppe\")
    fig.update_layout(
        title=dict(text=\"Materialanteil pro Bauteilgruppe\", font=dict(size=14, color=COLORS[\"text\"]),
                   x=0.0, y=0.98, yanchor=\"top\", xanchor=\"left\"),
...
def create_quality_gauge(score: float) -> go.Figure:
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            \"threshold\": {\"line\": {\"color\": COLORS[\"error_warning\"], \"width\": 4}, \"thickness\": 0.75, \"value\": score},
        },
        title={\"text\": \"Modellqualität\", \"font\": {\"size\": 16, \"weight\": \"bold\"}},
    ))
    fig.update_layout(paper_bgcolor=\"rgba(0,0,0,0)\", margin=dict(l=20, r=20, t=60, b=20), height=280)
    return fig
...
def create_error_bar(error_counts: dict) -> go.Figure:
    ...
    fig.update_layout(
        xaxis=dict(title=\"Anzahl\", range=[0, max_val * 1.25], showgrid=True, gridcolor=\"#F0F0F0\", zeroline=False),
...
def create_pset_matrix_heatmap(pset_matrix: pd.DataFrame) -> go.Figure:
    ...
    # Psets alphabetisch sortieren
    psets_sorted = sorted(pset_matrix.columns.tolist())
    # IFC-Klassen nach Anzahl vorhandener Psets absteigend (meiste links)
    completeness = (pset_matrix[psets_sorted] > 0).sum(axis=1)
    classes_sorted = completeness.sort_values(ascending=False).index.tolist()

    # Neu sortiertes DataFrame: Zeilen = IFC-Klassen, Spalten = Psets
    df_sorted = pset_matrix.loc[classes_sorted, psets_sorted]

    # z: Zeilen = Psets (Y), Spalten = IFC-Klassen (X)  → transponieren
    z = (df_sorted.T > 0).astype(int).values   # shape: (n_psets, n_classes)
    hover_text = [[\"Vorhanden\" if v else \"Fehlt\" for v in row] for row in z]

    fig = go.Figure(go.Heatmap(
        z=z,
        x=classes_sorted,      # X-Achse: IFC-Klassen
        y=psets_sorted,        # Y-Achse: Psets
